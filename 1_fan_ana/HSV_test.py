# -*- coding: utf-8 -*-
"""
Created on Sun Aug 29 17:59:42 2021
...
def hsv(x):
    img = cv.imread(x)
    src = cv.cvtColor(img, cv.COLOR_BGR2HSV)
    h,w,ch = np.shape(src)
    ...
    # kde = stats.gaussian_kde(hest_h)
    # print(kde)
    # # plt.plot(hest_s, color='g')
    # # plt.plot(hest_v, color='b')
    # plt.xlim([-10,180])
    # plt.show()
...
# =============================================================================
    ...
    # ax = fig.add_subplot()
    # ax.hist(hist, bins=180, color='blue', alpha=0.7)
